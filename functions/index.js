...
//triggers 5 minutes early to give time for function completion 
...
function sendArticlesAtTime(time, sendTrending = false){
    return new Promise(function fetchAndSend (resolve){
        var articlesForUsersDict = {}

        fetchArticles().then(function(value){
           // dictionary of all fetched articles
           let allFetchedArticles = value

           //send top trending to all users
            if (sendTrending){
                sendArticleToAllUsers(allFetchedArticles['Trending'][0]).then(function(){
                                    ...
                                    for (var articleIndex in articlesOfCategory){
                                        ...
                                        if (recentlyCuratedUrls.find(function(element) { return element == checkURL; })){
                                            ...
                                            articleRef.set(checkArticle).then(function(ref){
                                                ...
                                                groupMembers.forEach(function(memberUID){
                                                    if (memberUID in articlesForUsersDict){
                                                        if  (checkURL in articlesForUsersDict[memberUID]){
                                                            articlesForUsersDict[memberUID][checkURL][gId] = articleRef.id
                                                        
                                                        }
                                                        else {
                                                            articlesForUsersDict[memberUID][checkURL] = {}
                                                            articlesForUsersDict[memberUID][checkURL][gId] = articleRef.id
                                                        }
                                                        
                                                    }
                                                    else {
                                                        articlesForUsersDict[memberUID] = {}
                                                        articlesForUsersDict[memberUID][checkURL] = {}
                                                        articlesForUsersDict[memberUID][checkURL][gId] = articleRef.id
                                                    }
                                                })
        ...
        //need to wait for all the articles to be sent to groups in order for articleForUsersDict to have the information we need
        //this should be done with promises but it gets tricky with that many levels of promises
        //this seems to work fine for now but its not perfect and could be improved with promises at some point in the future
        //in testing the function only needs a few seconds to complete so giving it a 5 minute buffer should be more than enought to be safe    
        setTimeout(function(){

            for (userID in articlesForUsersDict){
                for (keys in articlesForUsersDict[userID]){
                    //all occurences of a specific article for a specific user
                    let copiesOfArticle = articlesForUsersDict[userID][keys]
                    let postGroupID = Object.keys(copiesOfArticle)[0]
                    let postArticleID = copiesOfArticle[postGroupID]


                    let groupData = Object.assign({}, copiesOfArticle)
                    var date = new Date();
                    ...
                    //post to homefeed
                    admin.database().ref('homeFeed/' + userID).push().set(newPost).then(function(r){
                    })  
...
//send an article to all valid users
function sendArticleToAllUsers (article){
    let articleRef = admin.firestore().collection("articles").doc();
    ...
    return articleRef.set(article).then(function(ref){
            return admin.database().ref("tks").once('value').then(function(users){
                users.forEach(user => {
                        ...
                        setTimeout(function(){
                            sendNotification(user.val(), "New Curated Article", "Foggy Glasses News curated a new Article for you!", articleRef.id, "Curated");
                        }, 300000);  
